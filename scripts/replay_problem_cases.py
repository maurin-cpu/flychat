"""
One-off Replay-Script: testet die 28 Spot-Problem-Cases gegen den aktuellen
Skill (Schranken 3+4 gestrichen, Vignetten eingefuegt, Rating-5 weicher).

Build a minimal user prompt that mirrors the production RATING-INPUTS path.
Other context elements (hourly synoptic, sector winds) are omitted — they
influence prose tags, not the rating decision itself.

Usage: python scripts/replay_problem_cases.py [--limit N]
"""
...
def build_user_message(case):
    """Build minimal user message mirroring production RATING-INPUTS path."""
    ...
    elev = (case.get("weather_input") or {}).get("elevation_m", "?")
    ...
    # Reproduziere Safety-Block-Output: laut Labels sind diese Cases alle "safe".
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=None, help="Nur erste N Cases")
    ...
    print(f"Replay {len(cases)} Problem-Cases gegen aktuelles Skill")
    ...
    for i, case in enumerate(cases, 1):
        ...
        print(f"[{i:>2}/{len(cases)}] {name:<35s} orig=5 gt={gt} new={new_rating} {matched}  "
              f"{recommendation}")
        results.append({
            "name": name, "orig": llm_orig, "gt": gt, "new": new_rating,
            "tier": case.get("terrain_tier"),
            "rec": recommendation,
        })
    ...
    print(f"Original LLM-Rating: alle 5 (Problem-Cases)")
    print(f"Pilot-GT-Verteilung: 4 (n=24), 3 (n=4) — kein einziger GT=5")
    print()
    print(f"Neue Rating-Verteilung nach Skill-Update:")
    ...
    print()
    cost_per_m_in = 0.27  # DeepSeek input
    cost_per_m_out = 1.10  # DeepSeek output
    est_cost = (total_in / 1e6 * cost_per_m_in) + (total_out / 1e6 * cost_per_m_out)
    print(f"Tokens: {total_in} in + {total_out} out = ~${est_cost:.3f}")
