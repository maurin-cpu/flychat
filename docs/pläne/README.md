...
| [`PLAN_ogn_validation.md`](PLAN_ogn_validation.md) | **Nicht begonnen** (erstellt 2026-06-14). Real-Flug-Validierung via OpenGliderNetwork (Live-APRS-Stream, FANET-Gleitschirme). Eigenständig, NICHT mit `validation/xcontest` zu vermischen. Noch kein Code (`ogn_collector.py`/`ogn_tracks.db`/`ogn_validation/` existieren nicht). Einstieg = Phase 0 Adoptions-Check als Gate. |
