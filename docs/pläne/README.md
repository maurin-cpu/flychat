...
| [`PLAN_ogn_validation.md`](PLAN_ogn_validation.md) | **Nicht begonnen** (erstellt 2026-06-14, methodisch überarbeitet 2026-08-14). Real-Flug-Belege via OpenGliderNetwork (Live-APRS-Stream, FANET-Gleitschirme). Einseitige Quelle: nur Anwesenheit zählt. Hauptzweck = Steig-/Höhen-Kalibrierung, nicht Fehlalarm-Jagd; Flugzahlen sind als Massstab verdorben. Zielort ist `validation/ogn/` (Hausordnung). Noch kein Code. Einstieg = Phase 0 Archiv-Check, danach Gate mit Zahlen. |
...
| [`PLAN_briefing_mail_v3.md`](PLAN_briefing_mail_v3.md) | **Als Vorschau umgesetzt** (2026-09-09). Mail folgt der Piloten-Kette, zählt Spots je Region statt sie zu küren, Synoptik-Karte mit DWD-Fronten. Lebt in `scripts/briefing_v3_context.py` + `templates/preview/briefing_preview_page.html`; **App-Code unverändert** — Übernahme in `email_service.py`/`briefing.html` erst auf Ansage. |
