...
End-to-End Smoke Tests stellen sicher, dass die wichtigsten Frontend-Flows der Flychat-App tatsächlich funktionieren — nicht nur die Backend-Endpoints, sondern der vollständige Pfad **Browser → Map-Klick → Overlay → SVG-Render**.
...
**Fix:** `initMap()` in `map.js` exportiert die Leaflet-Instanz explizit als `window.flychatMap`, und alle sechs Call-Sites in `index.html` verwenden jetzt diesen kollisionsfreien Namen.
