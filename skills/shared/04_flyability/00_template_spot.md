...
5b. **Region-Herleitung in `recommendation`**: Die `recommendation` MUSS die Region-Einschaetzung sichtbar als Quelle nennen ('Region sieht gut/schwach aus' + Region-Rating + Region-Basis/AGL-Zahl) und daraus lokal vs. Streckenflug ableiten. XC-/Lokal-Aussage ohne erkennbaren Region-Bezug = FEHLER.
...
```json
{
  ...
  "recommendation": "**Einschaetzung** (KEINE Empfehlung). 4-6 Saetze. NUR Flugqualitaet, KEIN Safety-Bezug (Hoehenwind, Boeen, Scherung, Foehn, Regen, Gewitter, 'sportlich' tabu). Satz 1: Was fuer ein Tag (aus rating). Satz 2-3: Begruendung aus RATING-INPUTS. **PFLICHT — Region-Herleitung sichtbar machen (1-2 Saetze):** Benenne die Region-Einschaetzung explizit als Quelle ('Region sieht gut aus' / 'Region sieht schwach aus') mit Region-Rating, Region-Thermik und Region-Basis/AGL als konkreter Zahl, und leite daraus lokal vs. Streckenflug ab. Der Leser MUSS verstehen, dass sich die Lokal-/XC-Aussage aus der REGION ergibt — nie XC/Strecke (auch nicht 'XC-Potenzial limitiert') ohne diese sichtbare Region-Begruendung. Satz 4: thermisches Fenster. Satz 5: ehrliche Erwartung. KEINE Aufforderungen. 'einschaetzen' statt 'empfehlen'. GUT (Region stark): 'Solider bis starker Thermiktag. Peak 2.2 m/s ueber 5h, AGL 1800m, Cu sauber 30%. Weil die Region heute stark aussieht (Region-Rating 5, hohe Basis ~2000m AGL), traegt es auch ab diesem Startplatz weg — Streckenflug 50-100km drin.' GUT (Region schwach): 'Kurzer aber lohnender Thermiktag, Peak 2.2 m/s ueber 4h. Weil die Region schwach und tief bleibt (Region-Rating 2, nur ~450m AGL), bleibt es ein lokaler Flug — Hausrunden und kurze Talquerungen statt Strecke.' SCHLECHT: 'XC-Potenzial durch die knappe Basis limitiert.' (XC-Aussage ohne sichtbaren Region-Bezug) oder 'Starker Tag, jedoch koennte Hoehenwind...' (Safety-Mischung verboten).",
