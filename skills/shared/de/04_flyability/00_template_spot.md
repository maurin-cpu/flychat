...
3a. **`Rating-Regel Flug` angewendet?** Wenn der Datenblock eine hat: Bedingung stundenweise gegen die Werte geprueft, Cap/Gate/Fenster-Wirkung umgesetzt, Ergebnis in `bemerkung_check` dokumentiert UND der Grund in der `recommendation` genannt. Rating-Regel vorhanden + `bemerkung_check` leer = FEHLER. Rating gekappt + recommendation nennt den Grund nicht = FEHLER.
