...
3a. **`Rating-Regel Flug` applied?** If the data block has one: condition checked hour by hour against the values, cap/gate/window effect applied, result documented in `bemerkung_check` AND the reason stated in the `recommendation`. Rating rule present + empty `bemerkung_check` = ERROR. Rating capped + recommendation does not state the reason = ERROR.
