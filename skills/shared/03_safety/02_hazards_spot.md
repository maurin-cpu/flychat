...
**Tags Spots (Hazards — Filter `[WIND-OK]`/`[WIND-WRONG]` werden separat in `_tagesfenster.md` behandelt):**
...
- `[GUST-DANGER]` / `[ALOFT-GUST-DANGER]` → DANGER-Niveau (> {{cfg.GUST_DANGER_KMH}} km/h). **Kein Auto-NoGo** — LLM entscheidet anhand von GUST-TREND und Fenster.
