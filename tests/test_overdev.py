"""Tests fuer die Ueberentwicklungs-Stufe (overdev.py + hohler Blitz).
...
def test_konsistenz_keine_wolken_keine_warnung():
    ...
    assert overdev.is_overdev_hour(CT_OK, data) is False
...
def test_lifted_index_ersetzt_cape():
    ...
    assert overdev.is_overdev_hour(CT_OK, data) is True
...
def test_blauthermik_veto():
    ...
    assert overdev.is_overdev_hour(CT_OK, DATA_OK, therm=therm) is False


def test_thermik_erreicht_basis_kein_veto():
    therm = {"climb_rate": 1.8, "max_height": 2600, "lcl": 2400}
    assert overdev.is_overdev_hour(CT_OK, DATA_OK, therm=therm) is True
...
def test_fehlende_thermik_blockiert_nicht():
    ...
    assert overdev.is_overdev_hour(CT_OK, DATA_OK, therm=None) is True
