"""Tests fuer die Ueberentwicklungs-Stufe (convection_rules.py + hohler Blitz).
...
def test_alle_bedingungen_erfuellt():
    assert convection_rules.is_overdev_hour(CT_OK, DATA_OK) is True


def test_harte_blitzstunde_gewinnt():
    """Ein Symbol, eine Botschaft: in einer Gewitterstunde kein Zusatzsymbol."""
    assert convection_rules.is_overdev_hour(CT_OK, DATA_OK, storm=True) is False


def test_ohne_wolkentop_nichts():
    assert convection_rules.is_overdev_hour(None, DATA_OK) is False


def test_zu_wenige_kalte_punkte():
    ct = {"cold_share_pct": config.OVERDEV_TOP_SHARE_PCT - 10, "top_min_c": -30.0}
    assert convection_rules.is_overdev_hour(ct, DATA_OK) is False
...
def test_konsistenz_keine_wolken_keine_warnung():
    ...
    assert convection_rules.is_overdev_hour(CT_OK, data) is False
...
def test_lifted_index_ersetzt_cape():
    ...
    assert convection_rules.is_overdev_hour(CT_OK, data) is True
...
def test_blauthermik_veto():
    ...
    assert convection_rules.is_overdev_hour(CT_OK, DATA_OK, therm=therm) is False


def test_thermik_erreicht_basis_kein_veto():
    therm = {"climb_rate": 1.8, "max_height": 2600, "lcl": 2400}
    assert convection_rules.is_overdev_hour(CT_OK, DATA_OK, therm=therm) is True
...
def test_fehlende_thermik_blockiert_nicht():
    ...
    assert convection_rules.is_overdev_hour(CT_OK, DATA_OK, therm=None) is True
